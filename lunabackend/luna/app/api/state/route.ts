...
export async function GET() {
  const [categories, items, orders, guests, ingredients, movements, expenses, zones, reservations, settings, promocodes] = await Promise.all([
    ...
    db.reservation.findMany({ include: { table: true }, orderBy: { date: "asc" } }),
  ...
  return NextResponse.json({ categories, items: availableItems, orders, guests, ingredients, movements, expenses, zones, reservations, settings, promocodes });
}
