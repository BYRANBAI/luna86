...
function Clients({ data, save }: any) {
  ...
  const exportClients = () => {
    const rows = [["ID", "Имя", "Телефон", "Подтверждён", "Email", "Бонусы", "Сегмент", "Заказов", "С сайта", "Дата"],
      ...(data.guests ?? []).map((g: any) => [g.id, g.name, g.phone, g.phoneVerified ? "да" : "нет", g.email ?? "", g.bonuses, g.segment, g.orders?.length ?? 0, g.registered ? "да" : "нет", new Date(g.createdAt).toLocaleDateString("ru-RU")])];
  ...
  return (
    <div>
      ...
      <div className="overflow-auto rounded-xl border border-[#EDEDED]">
        <table className="w-full text-left text-sm">
          <thead style={{ background: "#ffffff" }}>
            <tr>
              {["ID", "Имя", "Телефон", "Email", "Бонусы", "Сегмент", "Заказов", "С сайта", "Последний визит", ""].map(h => (
          ...
          <tbody>
            {guests.map((g: any) => (
              <tr key={g.id} className="border-t border-[#EDEDED] hover:bg-[#FFF3E6]">
                <td className="px-4 py-3 text-slate-400">{g.id}</td>
                <td className="px-4 py-3 font-semibold">{g.name}</td>
                <td className="px-4 py-3">
                  {g.phone}
                  {g.phoneVerified
                    ? <span className="ml-2 rounded-full bg-emerald-500/15 px-2 py-0.5 text-xs text-emerald-300">подтверждён</span>
                    : <span className="ml-2 rounded-full bg-amber-500/15 px-2 py-0.5 text-xs text-amber-300">ожидает код</span>}
                </td>
                ...
                <td className="px-4 py-3 text-slate-400">
                  {g.registered ? "да" : "нет"}
                  <div className="text-xs text-slate-500">{new Date(g.createdAt).toLocaleDateString("ru-RU")}</div>
                </td>
      ...
      {selected && editForm && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4">
          <div className="panel w-full max-w-2xl max-h-[90vh] overflow-auto p-6">
            ...
            <div className="mb-5 rounded-lg border border-[#EDEDED] bg-[#ffffff] p-4">
            ...
            <div className="mb-5 rounded-lg border border-[#EDEDED] bg-[#ffffff] p-4">
              <p className="mb-2 text-sm font-semibold">Телефон и регистрация</p>
              <p className="text-sm text-gray-600">{selected.phone} · {selected.phoneVerified ? "номер подтверждён" : "номер не подтверждён"}</p>
              <p className="muted mt-1 text-xs">{selected.registered ? "Зарегистрирован с сайта" : "Начал регистрацию, код ещё не введён"} · {selected.tags || "без тегов"}</p>
            </div>

            <div className="mb-5 rounded-lg border border-[#EDEDED] bg-[#ffffff] p-4">
              <p className="mb-2 text-sm font-semibold">Адреса · {selected.addresses?.length ?? 0}</p>
              {(selected.addresses ?? []).map((a: any) => (
                <p key={a.id} className="border-t border-[#EDEDED] py-2 text-xs text-gray-600">
                  {a.label}: {a.street}, {a.building}{a.apartment ? `, кв. ${a.apartment}` : ""}{a.isDefault ? " · основной" : ""}
                </p>
              ))}
              {(selected.addresses?.length ?? 0) === 0 && <p className="muted text-xs">Адресов пока нет</p>}
            </div>

            <div className="mb-5 rounded-lg border border-[#EDEDED] bg-[#ffffff] p-4">
              <p className="mb-2 text-sm font-semibold">История заказов · {selected.orders?.length ?? 0}</p>
              {(selected.orders ?? []).slice(0, 5).map((o: any) => (
                <p key={o.id} className="border-t border-[#EDEDED] py-2 text-xs text-gray-600">
            ...
            <div className="mb-5 rounded-lg border border-[#EDEDED] bg-[#ffffff] p-4">
              ...
              {passMsg && <p className="mt-2 text-xs text-gray-600">{passMsg}</p>}
            </div>
...
function SiteEditor({ data, save }: any) {
  ...
  const tabBtn = (key: typeof tab, label: string) => (
    <button key={key} onClick={() => setTab(key)} style={{ padding: "8px 18px", borderRadius: 10, border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer", background: tab === key ? "#F58220" : "transparent", color: tab === key ? "#151b27" : "#6B6B6B" }}>
      {label}
    </button>
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2 rounded-xl border border-[#EDEDED] bg-[#ffffff] p-1 w-fit">
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-auto rounded-xl border border-[#EDEDED]">
            <table className="w-full text-sm text-left">
              <thead style={{ background: "#ffffff" }}>
                <tr>{["Фото", "Название", "Цена", "Доставка", "Теги", "Сайт", ""].map(h => <th key={h} className="px-3 py-3 font-semibold text-gray-600">{h}</th>)}</tr>
              </thead>
              <tbody>
                {items.map((i: any) => (
                  <tr key={i.id} className="border-t border-[#EDEDED] hover:bg-[#FFF3E6]">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex gap-1 flex-wrap">
                        {["hit","new","spicy"].map(lbl => (
                          <button key={lbl} onClick={() => toggleLabel(i, lbl)} style={{ padding: "2px 8px", borderRadius: 12, fontSize: 11, fontWeight: 700, cursor: "pointer", border: "1.5px solid", borderColor: (i.labels ?? "").includes(lbl) ? "#F58220" : "#EDEDED", background: (i.labels ?? "").includes(lbl) ? "#FFF3E6" : "transparent", color: (i.labels ?? "").includes(lbl) ? "#F58220" : "#6B6B6B" }}>
                            {lbl === "hit" ? "🔥" : lbl === "new" ? "✨" : "🌶"}
                          </button>
                        ))}
                      </div>
      ...
      {tab === "banner" && (
        <Card>
          ...
          <div style={{ borderRadius: 16, background: `linear-gradient(135deg, ${bannerForm.color1} 0%, ${bannerForm.color2} 100%)`, padding: "28px 32px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <span style={{ background: "#F58220", color: "#fff", padding: "4px 12px", borderRadius: 7, fontSize: 12, fontWeight: 700, marginBottom: 10, display: "inline-block" }}>{bannerForm.badge || "АКЦИЯ"}</span>
        ...
        <Card>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            ...
            {promoItems.map((i: any) => (
              <div key={i.id} style={{ flexShrink: 0, width: 120, borderRadius: 12, overflow: "hidden", border: "2px solid #F58220", background: "#FFF3E6" }}>
                ...
                <div style={{ padding: "6px 8px" }}>
                  ...
                  <div style={{ fontSize: 12, fontWeight: 800, color: "#F58220" }}>{i.deliveryPrice ?? i.price} ₽</div>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="space-y-3">
              ...
              <div><label className="muted mb-1 block text-xs">Фото</label>
                <div className="flex gap-2 items-center mb-2">
                  <label style={{ cursor: "pointer", background: "#1c3146", border: "1.5px solid #EDEDED", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 700, color: "#f4efe5" }}>
...
function Recipes({ data }: any) { return <Card><h2 className="mb-4 font-bold">Техкарты и версии</h2>{(data.items ?? []).map((i: any) => <div className="border-b border-[#EDEDED] py-3" key={i.id}><b>{i.name}</b><span className="muted ml-4">себестоимость {i.cost} ₽ · food cost {Math.round(i.cost / i.price * 100)}%</span>{i.ingredients?.map((r: any) => <p className="muted ml-4 text-sm" key={r.id}>{r.ingredient.name} · {r.grams} {r.ingredient.unit}</p>)}</div>)}</Card>; }
